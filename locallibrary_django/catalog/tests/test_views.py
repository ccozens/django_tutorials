...
# User required to assign User as a borrower, Permission eequired to grant the permission needed to set a book as returned
...
class AuthorListViewTest(TestCase):
    ...
    def test_lists_all_authors(self):
        ...
        response = self.client.get(reverse("catalog:authors") + "?page=3")
        self.assertEqual(response.status_code, 200)
        self.assertTrue("is_paginated" in response.context)
        self.assertTrue(response.context["is_paginated"] == True)
...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_HTTP404_for_invalid_book_if_logged_in(self):
        ...
        login = self.client.login(username="testuser2", password="2HJ1vRV0Z&3iD")
        response = self.client.get(
            reverse("catalog:renew-book-librarian", kwargs={"pk": test_uid})
        )
...
#             },
#         )
#         # check we get a response "success"
#         # self.assertEqual(response.status_code, 200)
#         # check redirect to new author page
#         self.assertRedirects(
#             response, reverse("catalog:author-detail", kwargs={"pk": 1})
#         )

#     def test_author_create_forbidden_if_logged_in_but_not_correct_permission(self):
#         login = self.client.login(username="testuser1", password="1X<ISRUkw+tuK")
#         # check we get need to login with admin message
#         message = "You must be logged as admin in to create or edit an author."
#         response = self.client.get(reverse("catalog:author_create"))
#         self.assertEqual(response.status_code, 200)
#         # You must be logged as admin in to create or edit an author.
#         self.assertFormError(response, "form", "field", message)
